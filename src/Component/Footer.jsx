...
export const Footer = () => {
  ...
  return (
    <footer className="group relative w-full mt-10 overflow-hidden rounded-3xl border-2 border-white/10 bg-black/20 p-8 backdrop-blur-xl transition-all duration-500 hover:border-blue-500/50 hover:shadow-[0_0_40px_8px_rgba(59,130,246,0.1)]">
      ...
      <div className="relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          <div className="flex flex-col space-y-5">
            ...
            <p className="text-sm leading-relaxed text-slate-600 dark:text-white/60 max-w-xs">
              Building <span className="text-blue-500 font-medium">intelligent digital experiences</span> by merging MERN stack architecture with modern AI tools[cite: 1, 4, 19].
            </p>
          </div>

          {/* Column 2: Contact (Synced with Contact.jsx) */}
          <div className="flex flex-col space-y-4">
            <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-blue-500 dark:text-blue-400">Connectivity</h3>
            <ul className="space-y-4">
              {[
                ...
                <li key={i} className="flex items-center gap-3 text-sm text-slate-600 dark:text-white/70 group/link">
                  <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-500/10 border border-blue-500/20 text-blue-400 group-hover/link:bg-blue-500/20 transition-colors">
                    <item.icon size={14} />
                  </div>
                  {item.href ? (
          ...
          <div className="flex flex-col space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {['React.js', 'Node.js', 'MongoDB', 'Tailwind', 'AI Integrations'].map((tech) => (
                <span key={tech} className="px-3 py-1 text-[10px] font-bold rounded-xl bg-white/5 dark:bg-white/10 border border-white/10 text-slate-700 dark:text-white/80 hover:border-blue-500/50 transition-colors cursor-default">
                  {tech}
                </span>
              ))}
            </div>
        ...
        <div className="mt-16 pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex flex-col items-center md:items-start gap-1">
            ...
            <p className="text-[9px] text-slate-500 dark:text-white/20 uppercase">
              BSc IT Student • D.G. Ruparel College 
            </p>
          </div>
